...
function comments_to_post($before = '', $limit = 10) {
	...
	if ($commenters) {
		...
		$current_commenter_email = $commenter->comment_author_email;
			
					$output .= '
					<div id="comment-inline">
					<div id="comment-avatar">' . prologue_get_avatar( $commenter->comment_post_ID, $commenter->comment_author_email, 32 ) . '</div>
					<div id="comment-post">
					<div id="comment-content">' . $commenter->comment_content . '</div>
					<div id="comment-author">' . $commenter->comment_author . ' left a comment.</div>
					</div>
					</div>
					';
		...
		echo $before . '<div id="recent-comment-wrap">' . $output . '</div>';
		
	}
}
// End comments to post
...
<p><a class="alltags" href="<?php bloginfo( 'url' ); ?>/tag-archive">All tag archive</a><br><a class="allrss" href="<?php bloginfo( 'rss2_url' ); ?>">All Updates RSS</a></p>
...
/* Delete button on each post on user interface for logged in authors */
